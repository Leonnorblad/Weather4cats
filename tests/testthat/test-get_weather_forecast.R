...
test_that("Not a valid date", {
  ...
  expect_error(get_weather_forecast(city="Linkoping", date="26-09-2022"))  # Wrong format
  expect_error(get_weather_forecast(city="Linkoping", date="Hello world")) # Wrong format
  expect_error(get_weather_forecast(city="Linkoping", date=Sys.Date()+8))  # To long ahead
})
...
test_that("Not a valid area", {
  ...
  expect_error(get_weather_forecast(city="linkoping", date=Sys.Date())) # No uppercase for the first letter
})

test_that("Missing arguments", {
  expect_error(get_weather_forecast())                 # Missing city and date
  expect_error(get_weather_forecast(city="Linkoping")) # Missing date
...
test_that("Dimentions of output is correct", {
  expect_equal(dim(get_weather_forecast(city="Linkoping", date=Sys.Date())), c(24, 8)) # Dimensions
})

test_that("Name of colums in output is correct", {
  expect_equal(colnames(get_weather_forecast(city="Linkoping", date=Sys.Date())), # Correct colnames
